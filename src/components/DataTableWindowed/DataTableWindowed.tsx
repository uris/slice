...
import styles from './DataTableWindowed.module.css';
...
export function DataTableWindowed<T>(props: Readonly<DataTableProps<T>>) {
	const {
	...
	// compute which rows should actually mount, given virtualizeRows/rowHeight/overscanRows
	...
	useEffect(() => {
		...
		if (virtualizeRows && height === 'auto') {
			console.warn(
				'[DataTableWindowed] `virtualizeRows` needs a bounded `height` (e.g. a fixed px or vh value) to create a ' +
		...
		if (virtualizeRows && !getRowId) {
			console.warn(
				'[DataTableWindowed] `virtualizeRows` is on without `getRowId`. Rows fall back to index-based keys, which ' +
					'can misattach hover/focus state to the wrong row as the window scrolls - pass `getRowId` for stable identity.',
			);
		}
	}, [virtualizeRows, height, getRowId]);
