...
export interface DataTableProps<T> {
	...
	/* Fixed px height every rendered row is expected to have. Only read when `virtualizeRows` is
	 * true. Must match (or closely approximate) each row's real rendered height - too small leaves
	 ...
	 * paging don't flash blank space before new rows mount. Only read when `virtualizeRows` is true.
	 * Defaults to 6. */
	overscanRows?: number;
}
